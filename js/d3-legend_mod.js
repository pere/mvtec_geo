    ...
    1: [function(require, module, exports) {
        ...
        (function(global, factory) {
            ...
            function extent(array, f) {
                ...
                if (f == null) {
                    while (++i < n)
                        if ((b = array[i]) != null && b >= b) { a = c = b; break; }
                    while (++i < n)
                        if ((b = array[i]) != null) {
                            if (a > b) a = b;
                            if (c < b) c = b;
                    ...
                    while (++i < n)
                        if ((b = f(array[i], i, array)) != null && b >= b) { a = c = b; break; }
                    while (++i < n)
            ...
            function max(array, f) {
                ...
                if (f == null) {
                    while (++i < n)
                        if ((b = array[i]) != null && b >= b) { a = b; break; }
                    while (++i < n)
                        if ((b = array[i]) != null && b > a) a = b;
                } else {
                    while (++i < n)
                        if ((b = f(array[i], i, array)) != null && b >= b) { a = b; break; }
                    while (++i < n)
    ...
    13: [function(require, module, exports) {
        ...
        exports.default = {
            ...
            d3_addText: function d3_addText(svg, enter, labels, classPrefix, labelWidth) {
                ...
                svg.select("." + classPrefix + "legendCells").selectAll(".cell")
                enter.append('g').attr('class', 'text_container');

                svg.select("." + classPrefix + "legendCells").selectAll(".cell .text_container").append("text").attr("class", classPrefix + "label");

                //var labels=['First tiem//test','Sec item//test2'];
                if (labels[0].split('//').length > 1) {
                    debugger
                    var labels_new = labels.map(function(d) {
                        return d.split('//')[0]
                    });
                    var sec_labels = labels.map(function(d) {
                        return d.split('//')[1]
                    });
                } else {
                    var labels_new = labels;
                }

                console.log(sec_labels)

                var text = svg.selectAll("g." + classPrefix + "cell text." + classPrefix + "label").data(labels_new).text(d3_identity);

                /*
                var text = svg.selectAll("g." + classPrefix + "cell text." + classPrefix + "label").data(function(d) {
                    console.log(d)
                    return d;
                }).text(d3_identity);
                */
                ...
                if (sec_labels) {
                    ...
                    svg.select("." + classPrefix + "legendCells").selectAll(".cell .text_container").append("text").attr("class", classPrefix + "label_sec");

                    var sec_text = svg.selectAll("g." + classPrefix + "cell text." + classPrefix + "label_sec").data(sec_labels).text(d3_identity);

                    // console.log(svg.selectAll("g.legendCells .cell text.label_sec"))
                    // //alert(svg.select("g.text_container").size())
                    //     // svg.selectAll("g.text_container").selectAll("text.label_sec").data(sec_labels)
                    // var sec=svg.selectAll("g.text_container text.label_sec").data(sec_labels)
                    //     .enter().append('text').attr('class', 'label_sec').text(function(d) {
                    //         console.log('sdfsdf is ' + d)
                    //         return d
                    //     })

                    //console.info('sec_text elem is ', sec_text)
                    //.text(d3_identity);

                    // var e = svg.selectAll(".text_container text.label_sec");
                    // console.log(e)
                    // e.data(sec_labels, function(d)

                    //         {
                    //             console.log('sec is ' + d)
                    //             return d
                    //         }).enter()
                    //     .append('text')

                    // .attr('class', 'label_sec').text(sec_labels)

                    // sec_text_enter.enter().append('text').text(function(d) {
                    //         console.info('sec label is ' + d);
                    //         return 'my text';
                    //     })
                    // .append("text").attr("class", classPrefix + "legendTitle");

                    // svg.selectAll("text." + classPrefix + "legendTitle").text(title);

                    //.append("text").attr("class", classPrefix + "label_sec").text('test')
                }
                ...
                return text;
                //return { 'main_text': text, 'sec_text': 'niente' };
    ...
    14: [function(require, module, exports) {
        ...
        function size() {
            ...
            function legend(svg) {
                ...
                if (svg.selectAll('text.label_sec').size() > 0) {
                    var sec = svg.selectAll('text.label_sec').nodes();
                    var max = 0;
                    svg.selectAll('text.label').each(function(d, i) {
                        console.log(i)
                        console.log('clientrect', d3.select(this).node().getBoundingClientRect())
                        var w = d3.select(this).node().getBoundingClientRect().left;
                        if (w > max)
                            max = w;

                        // d3.select(sec[i]).attr('transform', 'translate(' + w + ',5)')

                    });
                    console.warn('max is ' + max)
                    svg.selectAll('text.label_sec').attr('transform', 'translate(' + max + ',5)')
                }
                cell.transition().style("opacity", 1);
            }
    ...
    15: [function(require, module, exports) {
        ...
        function symbol() {
            ...
            function legend(svg) {
                ...
                var sec = svg.selectAll('text.label_sec').nodes();
                alert('lengtg sec is ' + sec.length)
                svg.selectAll('text.label').each(function(d, i) {
                    console.log(i)
                    var w = d3.select(this).node().getBoundingClientRect().width + 30;
